...
import sys
import os
import configparser  # 读取文件模块

base_path = os.getcwd()
sys.path.append(base_path)
path = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))
file_path1 = os.path.join(path, "Config", "server.ini")
...
class HandleInit():
    ...
    # 获取ini里面的value,key存在就返回对应的value
    ...
    # 获取token操作
    def get_ope_token(self):
        return int(self.get_value('operate_token'))
